...
| 狀態 | 瀏海上的樣子 | 什麼時候 |
|---|---|---|
| 處理中 | 瀏海左側有隻像素吉祥物在走路 + 右側專案名 | Claude 正在思考或執行工具 |
| 等你確認 | 瀏海往下展開「等你確認 · 專案名」並常駐，吉祥物會定時跳一下提醒你 | Claude 在等你授權或回答 |
| 完成 | 展開「完成 · 專案名」3 秒，吉祥物蹲下、大跳迸出火花、再彈兩下，之後收起並睡著 | 這一輪跑完了 |
| 閒置 | 吉祥物閉眼睡覺、旁邊飄著 Z + 專案名 | 跑完之後在等下一個指令 |

畫面文字會跟隨 macOS 的語言設定：偏好語言是繁體中文時顯示中文，其他語言一律顯示英文（例如「Needs you」「Done」）。
...
- Claude Code 的 `idle_prompt` 通知不一定會觸發，所以「跑完了」的睡著吉祥物是由「完成」收起後直接顯示，而不是依賴它。
